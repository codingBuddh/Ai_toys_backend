...
async def process_audio_with_whisper(audio_file_path: str) -> str:
    ...
    try:
        ...
        with wave.open(audio_file_path, 'rb') as wf:
            ...
            logger.info(f"Processing {os.path.basename(audio_file_path)}: {duration:.2f} seconds, {file_size_mb:.2f} MB")
    ...
    try:
        ...
        if not transcript:
            logger.warning(f"No transcript generated for {os.path.basename(audio_file_path)}")
            return ""
            
        logger.info(f"Transcription completed in {transcribe_time:.2f}s: \"{transcript[:50]}{'...' if len(transcript) > 50 else ''}\"")
